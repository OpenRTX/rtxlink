...
impl std::fmt::Display for MemInfo {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            ...
            "{}_{}",
            unsafe {
                CStr::from_ptr(self.name.as_ptr() as *const i8)
                    .to_str()
                    .unwrap()
                    .replace(" ", "")
            },
...
pub fn send_cmd(opcode: Opcode, params: Vec<Vec<u8>>) {
    ...
    let mut cmd: Vec<u8> = vec![opcode as u8, params.len() as u8];
    for p in params {
...
pub fn wait_reply(opcode: Opcode) -> Vec<Vec<u8>> {
    ...
    if rx_opcode != opcode {
        eprintln!(
            "{}: mismatched opcode in FMP response!",
            "Error".bold().red()
        );
        return vec![];
    }
...
pub fn meminfo() -> Vec<MemInfo> {
    ...
    let mem_list = available_mem.iter().map(|m| MemInfo::from(m)).collect();
    mem_list
}
